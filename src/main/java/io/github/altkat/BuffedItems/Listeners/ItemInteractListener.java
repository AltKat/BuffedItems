...
public class ItemInteractListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (buffedItem == null || !buffedItem.isActiveMode()) return;
        ...
        if (plugin.getCooldownManager().isOnCooldown(player, itemId)) {
            double remaining = plugin.getCooldownManager().getRemainingSeconds(player, itemId);
            ...
            if (buffedItem.isVisualChat()) {
                ...
                player.sendMessage(ConfigManager.fromLegacy(rawMsg.replace("{time}", timeStr)));
            }
            ...
            if (buffedItem.isVisualTitle()) {
                ...
                player.showTitle(net.kyori.adventure.title.Title.title(
                        ConfigManager.fromLegacy(title.replace("{time}", timeStr)),
                        ConfigManager.fromLegacy(subtitle.replace("{time}", timeStr))
                ));
            }
    ...
    private void executeCommands(Player player, List<String> commands) {
        ...
        if (originalRule == null) originalRule = true;
    ...
    private void applyActiveEffects(Player player, BuffedItem item) {
        ...
        int durationTicks = item.getActiveDuration() * 20;
        if (durationTicks <= 0) durationTicks = 20;
    ...
    private void playConfiguredSound(Player player, String itemSound, String globalSound) {
        ...
        if (soundString == null || soundString.equalsIgnoreCase("NONE")) return;
        ...
        try {
            ...
            ConfigManager.sendDebugMessage(ConfigManager.DEBUG_INFO, () -> "[Sound] Invalid sound format: " + soundString);
        }
    }
}