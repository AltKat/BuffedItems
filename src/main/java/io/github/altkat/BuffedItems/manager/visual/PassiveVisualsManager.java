...
public class PassiveVisualsManager {
    ...
    public void updatePlayerVisuals(Player player, List<ActiveItemInfo> currentActiveItems) {
        ...
        for (ActiveItemInfo info : currentActiveItems) {
            ...
            // Check if this item is newly equipped in this slot
    ...
    private void handleOnEquip(Player player, BuffedItem item) {
        ...
        if (soundSettings.isEnabled() && soundSettings.getSound() != null && !soundSettings.getSound().equalsIgnoreCase("NONE")) {
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                try {
                    ...
                    try {
                        // First, try to resolve as a Bukkit enum for best compatibility
                        ...
                        // Fallback to playing as a string (for custom sounds, e.g. "custom:my_sound")
        ...
        if (visuals.getTriggerMode() == VisualTriggerMode.ON_EQUIP && bossBarSettings.isEnabled()) {
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                UUID uuid = player.getUniqueId();
                ...
                if (playerOneTimeBars.containsKey(itemId)) {
                    ...
                    bar = info.bar;
                    bar.name(titleComp); // Update title just in case placeholders changed
                    
                    // Cancel old removal task
        ...
        if (visuals.getTriggerMode() == VisualTriggerMode.ON_EQUIP && actionBarSettings.isEnabled() && actionBarSettings.getMessage() != null) {
            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                ...
                // If duration is set, clear it after some time
                if (actionBarSettings.getDuration() > 0) {
                    // This is a bit tricky, as there's no "hideActionBar" for a specific component.
                    // We can just send an empty one after the duration if this player's priority action bar hasn't changed.
                }
    ...
    private void updateContinuousBossBars(Player player, List<ActiveItemInfo> currentActiveItems) {
        ...
        for (ActiveItemInfo info : currentActiveItems) {
            ...
            if (visuals.getTriggerMode() != VisualTriggerMode.CONTINUOUS) continue;
    ...
    private void sendPriorityActionBar(Player player) {
        ...
        // Priority logic: MAIN_HAND > OFF_HAND > Armor Slots
        String[] priority = {"MAIN_HAND", "OFF_HAND", "HELMET", "CHESTPLATE", "LEGGINGS", "BOOTS"};
        
        for (String slot : priority) {
            ...
            if (abSettings.isEnabled() && abSettings.getMessage() != null) {
